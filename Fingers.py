...
# Manually setting up ROI for grabbing the hand.
roi_top = 20
roi_bottom = 300
roi_right = 300
roi_left = 600
...
def segment(frame, threshold=25):
    ...
    # Only need the threshold, and throw away the first item in the tuple with an underscore _
    _ , thresholded = cv2.threshold(diff, threshold, 255, cv2.THRESH_BINARY)

    # Grab the external contours from the image
    image, contours, hierarchy = cv2.findContours(thresholded.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # If length of contours list is 0, then didn't grab any contours!
    if len(contours) == 0:
        return None
    else:
        #Only Hand will be grabed
